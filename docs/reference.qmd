...
`par` is a boolean, so `par="false"` turns it off and `par="true"` turns it on. Pandoc needs a value here: an attribute written without one is not parsed, and the span is left as text.
